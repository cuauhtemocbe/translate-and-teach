...
describe('TranslationTimer', () => {
  describe('when elapsedTime is null', () => {
    it('should render nothing when loading', () => {
      const { container } = render(<TranslationTimer isLoading={true} elapsedTime={null} />);
      expect(container.firstChild).toBeNull();
    });

    it('should render nothing when not loading', () => {
      const { container } = render(<TranslationTimer isLoading={false} elapsedTime={null} />);
      expect(container.firstChild).toBeNull();
    });
  });
  ...
  describe('when completed', () => {
    it('should show "Translation completed in" with elapsed time', () => {
      render(<TranslationTimer isLoading={false} elapsedTime={3.7} />);

      expect(screen.getByText(/Translation completed in 3\.7s/)).toBeInTheDocument();
    });

    it('should format time to one decimal place', () => {
      render(<TranslationTimer isLoading={false} elapsedTime={2.999} />);

      expect(screen.getByText(/Translation completed in 3\.0s/)).toBeInTheDocument();
    });

    it('should apply completed modifier class', () => {
      const { container } = render(<TranslationTimer isLoading={false} elapsedTime={2.5} />);
    ...
    it('should handle zero elapsed time', () => {
      render(<TranslationTimer isLoading={false} elapsedTime={0.0} />);
