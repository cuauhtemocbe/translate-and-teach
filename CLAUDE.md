...
2. **Local husky hooks** (`.husky/`): fast feedback *before* code reaches CI.
   ...
   - `pre-merge-commit` / `pre-push`: on `main`/`develop`, runs the full `scripts/validate.sh` (lint → typecheck → `test:coverage` → build → `pnpm audit`); on feature branches, a lighter typecheck-only check. `pre-push` additionally starts, on **every** push and before those checks, with a fail-closed Trivy scan (`trivy fs . --scanners vuln --severity CRITICAL --ignore-unfixed`): a CRITICAL dependency CVE that has a fix available blocks the push, and so does a missing `trivy` binary — install it locally first (see `.claude/skills/trivy-scan/setup.md`). It complements rather than duplicates the existing checks: `pnpm audit` only runs on pushes to `main`/`develop` and only covers the npm ecosystem, and `gitleaks` scans for secrets, not vulnerable dependencies. Rollout context: `specs/trivy-pre-push-cve-gate.md`.
